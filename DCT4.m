% changed dct with new quantisation table
% embed in lower frequency 
% larger hufman length
...
quant1=int64(zeros(size(I1,1),size(I1,1),1));
for k=1:1
    for i=1:block_size:size(I1,1)
        for j=1:block_size:size(I1,2)
            for ii=1:block_size
...
for ii = 1:Mm
    for jj = 1:Nm
        temp1cipher(ii,jj)=bitget(tempout(keymat(Nm*(ii-1)+jj)),1);
    end
end
