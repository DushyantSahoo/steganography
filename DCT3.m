% changed quantisation table for increasing the capacity for encryption
% also increases the PSNR
% embed in higher frequency 
% resulting larger hufman length
...
for i=1:block_size:size(I1,1)
    for j=1:block_size:size(I1,2)
        for ii=1:block_size
            for jj=1:block_size
                if ii+jj>5
                    keymat1(tempj)=Nm*(i+ii-1)+j+jj-1;
                    tempj=tempj+1;                    
                end
            end
        end
